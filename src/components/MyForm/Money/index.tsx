        ...
        <InputNumber
          ...
          formatter={ // 转换为金额格式，带千分符
            value => {
              ...
              return Number(value).toLocaleString('zh', { minimumFractionDigits: props.precision || 2 })
            }
          }
          parser={
            value => {
              ...
              if ((value as string).indexOf('.') > 0) {
                ...
                const integerVal = newValue.split('.')[0].replace(/\$\s?|(,*)/g, '').substr(0, props.integerPrecision || 13);
                const decimalsVal = newValue.split('.')[1].replace(/\$\s?|(,*)/g, '').substr(0, props.precision || 2);
