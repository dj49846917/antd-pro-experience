...
function Panel({}: Props) {
  ...
  useEffect(() => {
    if (!utils.isEmptyObject(bpmnjsModeler)) {
      ...
      const root = elementRegistry.filter((x: { type: EventType }) => x.type === EventType.PROCESS);
      dispatch({
        type: ActionType.changeCurrentElement,
        payload: { ...state, currentElement: root[0] || {} },
      });
      bpmnjsModeler.on('selection.changed', (e: { newSelection: string | any[] }) => {
        ...
        dispatch({
          type: ActionType.changeCurrentElement,
          payload: { ...state, currentElement: current },
        });
      });
    }
  }, [bpmnjsModeler]);
  ...
  return (
    <div className={styles['bpmn-panel']}>
      ...
      <Form form={form} autoComplete="off" layout="vertical" className={styles['bpmn-form']}>
        <Tabs
          className={styles['bpmn-tabs']}
          activeKey={state.activeTab}
          onChange={(e) =>
            dispatch({ type: ActionType.changeActiveTab, payload: { ...state, activeTab: e } })
          }
          size="small"
          items={tabInfo}
        />
