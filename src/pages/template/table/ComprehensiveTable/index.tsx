...
function ComprehensiveTable() {
  ...
  const action = (actionType: 'add' | 'edit', record?: TableListType) => {
    ...
    if (actionType === 'edit') {
      ...
      dispatch({
        type: ActionType.changeActiveRow,
        payload: { ...state, activeRow: record as TableListType },
      });
  ...
  const columns = [
    {
      ...
      key: 'ReportNm',
      render: (text: string) => {
        const list = state.dicList.filter(
  ...
  return (
    <PageContainer>
      <ComprehensiveTableContext.Provider value={{ state, dispatch }}>
        <Button type="primary" onClick={() => action('add')}>
          新增
        </Button>
        <Table
          pagination={{
